...
- For normal-node and jump-node talktrack work, Advanced Settings large model intent recognition 2.0 is mandatory. Configure the IVR-level switch with `POST <resolved-api-base>/ivr/updateModelIntentRecognitionConfig`; the payload `id` must equal the IVR ID, `modelIntentRecognitionEnabled=1`, `modelIntentRecognitionTimeoutMilliSecond=2000`, `modelMaxTokens>=4096`, and `modelRecognitionRound=0`. Use the model required by the resolved backend: domestic `modelId=55` / `闪电26BMoE-fast`; overseas / international `modelId=62` / `openai/gpt-5.4-mini`. Do not hardcode the domestic host or domestic model for overseas runs.
...
- Every normal node or jump node that uses large model intent recognition 2.0 must use the resolved backend's approved model: domestic `modelId=55` / `modelIntentRecognitionConfig.modelConfig.id=55` / `闪电26BMoE-fast`; overseas `modelId=62` / `modelIntentRecognitionConfig.modelConfig.id=62` / `openai/gpt-5.4-mini`. Do not rely on page defaults or first-node inheritance. When preserving an existing node-level config, force the model ID to the expected regional value in backend `sceneList`, frontend `sceneListFrontend.nodeList`, and graph `customData`, while preserving the node's prompt and result format.
- For every normal node or jump node that uses large model intent recognition 2.0, the node-level recognition round must default to all rounds: `modelIntentRecognitionConfig.modelConfig.recognitionRound=0`, which the page shows as `识别节点轮次：全部轮次`. Do not leave copied/template values such as `1`, and do not only fix the first node. Read back backend `sceneList`, frontend `sceneListFrontend.nodeList`, and graph `customData`; any enabled/configured node with `recognitionRound` missing or not `0` is incomplete unless the backend truly does not expose the field.
...
- Keep backend route lists and frontend canvas intent lists in their own shapes. Backend `sceneList.nodeList[].intentList` uses route dictionaries such as `{"27620":"node-xxx"}` / `{"-2":""}` / `{"-1":"node-yyy"}`. Frontend `sceneListFrontend.nodeList[].intentList` and graph `data.customData.intentList` must use option rows with `value`, `label`, and `digitSequence`, for example `{"value":"27620","label":"客户肯定/默认","digitSequence":""}`. Do not deep-copy backend nodes into frontend canvas data without converting `intentList`; the page save logic reads `value` from the frontend list and can rebuild broken routes, causing generic `system error` on save. Any frontend row that is a single-key backend route dictionary, such as `{"27620":"node-xxx"}`, is a P0 save-safety failure and must be normalized before writing or delivery.
- Enforce current-IVR intent ownership as a separate P0 gate. Before every `updateSceneList`, source-IVR copy, repair, or delivery, read `GET /ivrIntent/findList/{ivrId}` and build the authoritative positive intent-ID set for that exact IVR. Every positive numeric reference in backend `sceneList.nodeList[].intentList`, frontend `sceneListFrontend.nodeList[].intentList`, graph `data.customData.intentList`, and interrupted-intent rows must belong to it; only reserved system IDs such as `-1` and `-2` bypass the ownership check. A foreign ID may be structurally valid yet crash runtime routing before large-model 2.0 or TTS and may also crash IVR copy. Never carry source positive IDs into a copied IVR: remap by exact intent name using the target IVR catalog, require a unique match, and stop rather than guess, delete the branch, or write an unresolved ID.
...
3. Resolve and validate the backend before reads or writes. Use the user's URL when present; otherwise probe the pasted token against domestic and overseas `/account/findInfo`. Use the resolved `apiBase`, `webBase`, and `scriptGraphUrl` for the rest of the run.
4. Snapshot the target IVR with `/ivr/findSceneList/{ivrId}` before changing it, then read `/ivrIntent/findList/{ivrId}` as the current-IVR ownership authority. For copy tasks, validate the source before copy and read both source and target intent catalogs before remapping.
...
6. For jump nodes and routed normal nodes, keep target node IDs, ports, backend `sceneList`, frontend `sceneListFrontend`, and graph custom data consistent. Preserve the format split: backend intent routes stay as `{intentId: targetNodeId}`, while frontend canvas intent lists stay as `{value,label,digitSequence}` option rows. Validate every positive numeric intent ID against the exact target IVR catalog before write; copied routes must be remapped by exact intent name.
...
10. Enable Advanced Settings large model intent recognition 2.0 with `POST <resolved-api-base>/ivr/updateModelIntentRecognitionConfig`. Use the target IVR ID as `id`, set `modelIntentRecognitionEnabled=1`, use the approved scene-specific `modelPrompt` and `modelResultFormat`, choose the regional model ID, timeout `2000`, `modelMaxTokens>=4096`, and `modelRecognitionRound=0`. Domestic uses `modelId=55`; overseas uses `modelId=62`. The prompt/result format must not include `兜底` as an output intent.
11. Enable or preserve node-level large model intent recognition 2.0 on every normal node that handles user replies. Preserve existing valid node prompts and result formats, but always normalize the node-level model to the regional model ID when the node has 2.0 enabled or a 2.0 config, and force node-level recognition round to all rounds. Domestic expects `modelIntentRecognitionConfig.modelConfig.id=55`; overseas expects `modelIntentRecognitionConfig.modelConfig.id=62`; both expect `modelIntentRecognitionConfig.modelConfig.recognitionRound=0`. Create a node-level `modelIntentRecognitionConfig` from the approved template when it is missing.
12. Read back with `/ivr/findSceneList/{ivrId}` plus `/ivrIntent/findList/{ivrId}` and knowledge-base list/detail endpoints when answers are touched. For every positive numeric route reference, validate current-IVR ownership across backend nodes, frontend node copies, graph `customData`, and interrupted-intent rows. For knowledge-base matching and node-level 2.0, validate backend nodes, frontend node copies, and graph `customData`; for node-level 2.0 model selection and recognition round, validate every enabled/configured normal or jump node reads back with the expected regional model ID and `recognitionRound=0` in all three copies; for frontend canvas intent lists, validate every intent row has `value`, `label`, and `digitSequence`, and reject any frontend row shaped like `{intentId: targetNodeId}`; for graph rendering, validate frontend graph node cells still have page-native `keypadPort` ports and were not rewritten to generic `in` / `out`; for Advanced Settings 2.0, validate the IVR-level fields and page echo.
...
- Intent ownership audit: verify every positive numeric route reference belongs to the exact IVR returned by `/ivrIntent/findList/{ivrId}`; classify foreign or stale IDs as P0 and block runtime/copy acceptance.
