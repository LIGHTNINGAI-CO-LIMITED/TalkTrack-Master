...
"""
MODEL_INTENT_RECOGNITION_MODEL_BY_REGION = {
...
def expected_model_intent_model(region: str) -> dict[str, Any]:
    ...
    return MODEL_INTENT_RECOGNITION_MODEL_BY_REGION[region]


def is_expected_model_id(value: Any, expected_model: dict[str, Any]) -> bool:
    try:
        return int(value) == int(expected_model["id"])
...
def normalize_model_intent_config(node: dict[str, Any], result_label: str = "肯定/默认", model_id: int = DEFAULT_MODEL_ID) -> None:
    template = make_model_intent_config(result_label, model_id)
    ...
    for key, value in template["modelConfig"].items():
        if model_cfg.get(key) in (None, ""):
            model_cfg[key] = value
    # Node-level 2.0 uses the model required by the resolved backend.
    # Existing prompts and result formats are preserved.
...
def enable_model_intent_2(node: dict[str, Any], result_label: str = "肯定/默认", model_id: int = DEFAULT_MODEL_ID) -> None:
    node["modelIntentRecognitionEnabled"] = 1
    normalize_model_intent_config(node, result_label, model_id)
...
def update_ivr_model_intent_recognition_config(client: Client, ivr_id: int, expected_model: dict[str, Any]) -> dict[str, Any]:
    payload = make_ivr_model_intent_payload(ivr_id, model_id=int(expected_model["id"]))
    ...
    return {
        ...
        "modelIntentRecognitionPayloadSummary": {
            ...
            "modelIntentRecognitionEnabled": 1,
            "modelIntentRecognitionTimeoutMilliSecond": 2000,
            "modelId": expected_model["id"],
            "modelName": expected_model["name"],
            "modelMaxTokens": 4096,
            "modelRecognitionRound": 0,
...
def set_system_tts_node(node: dict[str, Any], name: str, text: str, tts_path: str, *, node_type: int, model_id: int = DEFAULT_MODEL_ID) -> None:
    ...
    if node_type == 1:
        ...
        enable_model_intent_2(node, model_id=model_id)
    else:
...
def configure_scene(client: Client, ivr_id: int, parsed: dict[str, Any], voice_id: int, speech_rate: float, expected_model: dict[str, Any]) -> tuple[dict[str, Any], list[dict[str, Any]], dict[str, Any]]:
    ...
    for node_spec in parsed["normalNodes"]:
        ...
        set_system_tts_node(node, node_spec["name"], node_spec["spokenText"], tts.get("recordFilePath") or "", node_type=1, model_id=int(expected_model["id"]))
...
def validate_run(ivr_id: int, parsed: dict[str, Any], scene_readback: dict[str, Any], node_tts: list[dict[str, Any]], kb_tts: list[dict[str, Any]], kb_details: list[dict[str, Any]], scene_meta: dict[str, Any], expected_model: dict[str, Any]) -> dict[str, Any]:
    ...
    if not is_expected_model_id(root_model_intent_summary.get("modelId"), expected_model):
        failures.append(f"IVR-level modelId != {expected_model['id']} ({expected_model['name']})")
...
def run_online(args: argparse.Namespace, parsed: dict[str, Any], token_text: str) -> dict[str, Any]:
    ...
    expected_model = expected_model_intent_model(backend["region"])
    ...
    report = {
        ...
        "modelIntentRecognitionExpectedModel": expected_model,
        "ivrId": ivr_id,
        "ivrName": ivr_name,
